...
export class ClientController {
  ...
  async uploadFiles(@UploadedFiles() files, @Response() res, @Body() body: BodyDto, @Query() query) {
    ...
    if (files) {
      const result = await this.clientService.uploadFromFile(files, accountId, query.suppress, query.cabinet);
      return res.json(result);
    }
    if (body.domains) {
      const result = await this.clientService.uploadFromBody(body.domains, accountId, query.suppress, query.cabinet);
      return res.json(result);
    }
  }
}
